...
function check(){
	...
	global $all_fine;
	global $user_exists;
	//$sql = "SELECT Account_No, Customer_Name, Balance, Contact_No FROM User_data WHERE Account_No = '$account_no'"; #MySQL Query
	...
	if($all_fine!=0&&$user_exists==0){
		...
		$query_users = "INSERT into Login VALUES('$account_no','$pin_no')";
		$conn->query($query) ;
